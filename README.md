...
| `--tree-sitter` | `false` | Use tree-sitter to resolve end lines when ctags does not provide them (supports Go `.go`, Python `.py`, Ruby `.rb`, Rust `.rs`, JavaScript `.js`, TypeScript `.ts`, Haskell `.hs`, OCaml `.ml`/`.mli`, PHP `.php`, and Kotlin `.kt`) |
...
2. Tree-sitter analysis (when `--tree-sitter` is enabled and the file is a supported language: `.go`, `.rs`, `.js`, `.ts`, `.hs`, `.ml`, `.mli`, `.php`, or `.kt`).
